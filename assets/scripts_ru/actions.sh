...
if [ -n "$maj" ];then
echo "Обновления выполняются..."
exit;fi
wid=$(xdotool search --onlyvisible --class "q4os-pkui")
if [ -n "$wid" ];then
echo "Доступны обновления."
echo " "
echo "Вы можете установить их, нажав"
echo "на значок менеджера обновлений"
echo "в области уведомлений панели задач."
else
echo "Ваша система обновлена."
fi
}
